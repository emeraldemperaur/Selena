...
def explorer_driver(arch):
    ...
    if local_host == "Windows":
        ...
        selena_x_driver_output = f"\t ...Executing Microsoft Edge Driver"
        selena_os_output_b = f"\t\tNOTE: Only supports Microsoft Edge (Chromium), install may be required"
        ...
        _selena_log_listener.info(selena_os_output_b)
        print(selena_os_output_b)
        executable_file_path = f"{virgil}\\{arch}\\windows\\msedgedriver.exe"
        x_driver = webdriver.Edge(executable_path=executable_file_path)
        ...
        _selena_log_listener.info(selena_os_output_b)
        print(selena_os_output_b)
        executable_file_path = f"{virgil}/{arch}/darwin/msedgedriver"
        x_driver = webdriver.Edge(executable_path=executable_file_path)
        return x_driver
    elif local_host == "Linux":
        selena_os_output = f"-->> Localhost: Linux Operating System ({platform.architecture()[0]})\n"
        selena_os_output_b = f"\tNOTE: Microsoft Edge on Linux is still under development...Please visit: https://www.microsoftedgeinsider.com/en-us/"
        _selena_log_listener.info(selena_os_output)
        print(selena_os_output)
        time.sleep(3)
        _selena_log_listener.info(selena_os_output_b)
...
def version_sleuth(found_driver):
    ...
    raw_driver_version = found_driver.capabilities['msedge']['msedgedriverVersion'].split(' ')[0]
    browser_version_str = raw_browser_version[0:2]
    driver_version_str = raw_driver_version[0:2]
    selena_web_version_output = f"\t\tMicrosoft Edge Web Version # -->> {raw_browser_version}"
    selena_driver_version_output = f"\t\tMicrosoft Edge Driver Version # -->> {raw_driver_version}\n"
    ...
    if int(browser_version_str) > int(driver_version_str):
        selena_response_a = f"\t\t Detected MSEdge Browser {browser_version_str} is higher than the found MSEdge Driver {driver_version_str}"
        selena_response_b = f"\t\t Please download an up-to-date Chrome Driver for {browser_version_str}"
        selena_response_c = "https://developer.microsoft.com/en-us/microsoft-edge/tools/webdriver/"
        ...
        selena_response_a = f"\t\t Found MSEdge Driver {driver_version_str} is higher than the detected MSEdge Browser {browser_version_str}"
        selena_response_b = "\t\t That's sort of an anomaly...You'll need to update your MSEdge browser to a newer version resolve"
        selena_response_c = "https://www.microsoft.com/en-us/edge"
        _selena_log_listener.warn(selena_response_a)
        print(selena_response_a)
        _selena_log_listener.warn(selena_response_b)
        print(selena_response_b)
